...
1. Aggregation & Math Accuracy:
This tool uses RAG, which retrieves the most relevant parts of a document, not the entire document.
...
2. Scanned Documents:
The system currently supports digital PDFs only (files where you can highlight text). Scanned image-based PDFs (photocopies) will return an error as no OCR engine is currently integrated.
